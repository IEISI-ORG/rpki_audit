    [*] Loading Global Audit for GU...

    ====================================================================================================
     NATIONAL ROUTING SECURITY: GU
    ====================================================================================================
    Total Networks:      10
    Total Cone Gravity:  6
    ------------------------------------------------------------
    SECURE (ACTIVE/PASSIVE):     2 (20.0%) -> Protects 33.3% of Traffic
    VULNERABLE NETWORKS:         5 (50.0%) -> Exposes  66.7% of Traffic

    ====================================================================================================
     THE GU CORE (Top 20 Networks)
    ====================================================================================================
    ASN      | Verdict                        | Cone     | APNIC% | Name
    ----------------------------------------------------------------------------------------------------
    AS3605   | REGRESSED                      | 4        | 0%     | Guam Cablevision, LLC.
    AS395400 | ACTIVE LOCAL ROV               | 2        | 100%   | UNIVERSITY OF GUAM
    AS9246   | STUB: VULNERABLE               | 0        | 0%     | Teleguam Holdings, LLC
    AS9548   | NOT ROUTED                     | 0        | -      | InternetPCI
    AS17456  | STUB: VULNERABLE               | 0        | -      | Pacific Data Systems
    AS23676  | STUB: PASSIVE (Clean Pipe)     | 0        | -      | Mariana Islands Internet Exchange
    AS55863  | NOT ROUTED                     | 0        | -      | Choice Phone LLC
    AS56200  | STUB: VULNERABLE               | 0        | -      | Guam Community College
    AS133071 | NOT ROUTED                     | 0        | -      | DoCoMo Pacific
    AS152735 | STUB: VULNERABLE               | 0        | -      | Guam Exchange

    ====================================================================================================
     TRANSIT SUPPLY CHAIN (Who provides to GU?)
    ====================================================================================================
    Rank | Upstream | Dependents | Global Status                  | Name
    ----------------------------------------------------------------------------------------------------
    #1   | AS7131   | 4          | REGRESSED                      | PTI Pacifica Inc.
    #2   | AS3356   | 2          | CORE: ACTIVE PROTECTOR         | Lumen (Level 3)
    #3   | AS2914   | 1          | CORE: ACTIVE PROTECTOR         | NTT America, Inc.
    #4   | AS17456  | 1          | STUB: VULNERABLE               | Pacific Data Systems
    #5   | AS3605   | 1          | REGRESSED                      | Guam Cablevision, LLC.
    #6   | AS4637   | 1          | CORE: ACTIVE PROTECTOR         | Telstra International Limited
    #7   | AS174    | 1          | CORE: ACTIVE PROTECTOR         | Cogent Communications, LLC
    #8   | AS140627 | 1          | PARTIAL: VULNERABLE (Mixed)    | OneQode
    #9   | AS395400 | 1          | ACTIVE LOCAL ROV               | UNIVERSITY OF GUAM
