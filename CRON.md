...
### `reports`
Runs `do_reports` — `rov_no_scrape_v22.py` (main audit) + all analysis scripts (`analyze_roa_signing_v2.py`, `analyze_herd_immunity_v2.py`, `analyze_roa_strategy_v3.py`, `analyze_aspa_realistic_v5.py`, `analyze_roa_signing_trends.py`, `analyze_aspa_real_deployment.py`, `aspa-path-protection`) + `statistics_v6.py` + per-country deep dives + pandoc HTML→MD conversion. `analyze_aspa_readiness_v2.py` was retired 2026-09-14 and `analyze_rov_quadrants_v4.py` was retired 2026-09-14 — see TODO.md. Uses cached topology from the last `full` run — does **not** touch ROA data or rebuild topology. Wrapped in a 3-hour `timeout`; a timeout or non-zero exit is logged as a warning but doesn't fail the cron slot (partial output is still useful).
